...
const GrenadeMapTab = ({ 
  ...
  hasLevels = false,
  zThreshold = null,
  grenadeData: externalGrenadeData,
  summary: externalSummary,
  matchesAnalyzed: externalMatchesAnalyzed,
  ...
  const { 
    ...
    matchesAnalyzed, 
    loading 
  } = useGrenadeStats(mapName, { enabled: !externalGrenadeData });

  const activeGrenadeData = externalGrenadeData || grenadeData;
  const activeSummary = externalSummary || summary;
  const activeMatchesAnalyzed = externalMatchesAnalyzed ?? matchesAnalyzed;
  ...
  const filteredClustersByType = useMemo(() => {
    return filterGrenadeClusters({
      grenadeData: activeGrenadeData,
      activeSide,
      hasLevels,
      zThreshold,
      currentLevel,
  ...
  }, [activeGrenadeData, activeSide, hasLevels, zThreshold, currentLevel, mapName]);
  ...
  const hasData = visibleClusters.length > 0 || Object.values(activeSummary).some(s => s?.thrown > 0);
  ...
  return (
    <div className="grenade-intel-panel">
      ...
      <GrenadeArsenal 
        ...
        matchesAnalyzed={activeMatchesAnalyzed}
...
export const GrenadeOverlay = ({ 
  ...
  const { grenadeData, loading } = useGrenadeStats(mapName, { enabled: !externalGrenadeData });
  const activeGrenadeData = externalGrenadeData || grenadeData;
  const activeLoading = externalLoading ?? loading;
  const hasLevels = zThreshold !== null && zThreshold !== undefined;
  ...
  }, [activeGrenadeData, activeSide, visibleTypes, hasLevels, zThreshold, currentLevel, mapName]);

  const clusteredVisibleByType = useMemo(() => {
    const output = {};

    Object.entries(filteredClusters).forEach(([type, clusters]) => {
      const baseVisible = mapName === 'de_nuke' ? 5 : mapName === 'de_vertigo' ? 5 : 6;

      output[type] = processGrenadeClustersForDisplay({
        clusters,
        mapName,
        grenadeType: type,
        zoomLevel,
        maxVisible: baseVisible,
      });
    });

    return output;
  }, [filteredClusters, mapName, zoomLevel]);
  ...
  return (
    <div className="grenade-overlay-intel">
      ...
      {/* Smart markers */}
      {Object.entries(clusteredVisibleByType).map(([type, clusters]) => {
        if (!visibleTypes[type]) return null;
        
        return clusters.map((cluster, idx) => (
          <GrenadeMarkerSmart
            key={`${type}-${cluster.x}-${cluster.y}-${idx}`}
            cluster={cluster}
            type={type}
